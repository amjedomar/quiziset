...
export class QuizController {
  ...
  @ApiOperation({
    summary:
      'get a paginated list of quizzes (public or managed/favorited by the current user) ' +
      'with optional search and sorting',
  })
  @ApiResponsesList({ status: 200, type: PaginatedQuizzesEntity }, 401)
  getAllQuizzes(
    @Query() query: GetAllQuizzesQueryDto,
    @AuthUser() user?: AuthUserData,
  ): Promise<PaginatedQuizzesEntity> {
